...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None):
        ...
        for lvl, (src, mask, pos_embed) in enumerate(zip(srcs, masks, pos_embeds)):
            ...
            src = src.flatten(2).transpose(1, 2)   # src: [N, Hi*Wi, C]
            mask = mask.flatten(1)   # mask: [N, Hi*Wi]
            pos_embed = pos_embed.flatten(2).transpose(1, 2)  # pos_embed: [N, Hp*Wp, C]
        ...
        # # memory = memory [res 5]
        # # TODO: is res5 enough?
        # bs, c, h, w = srcs[(self.num_feature_levels-2)*self.num_frames].shape
        # st_index = int(level_start_index[(self.num_feature_levels - 2) * self.num_frames])
        # end_index = int(level_start_index[(self.num_feature_levels - 1) * self.num_frames])

        # memory = memory[:, st_index:end_index, :]
        # memory = memory.reshape(-1, memory.shape[-1]).view(bs * self.num_frames, h, w, c).permute(0, 3, 1, 2)
        # # memory: [bs*nf, c, h, w]

        return hs, memory, init_reference_out, inter_references_out, inter_samples, None, None
...
class DeformableTransformerEncoderLayer(nn.Module):
    ...
    def forward(self, src, pos, reference_points, spatial_shapes, level_start_index, padding_mask=None):
        # self attention
        src2, sampling_locations, attention_weights = self.self_attn(self.with_pos_embed(src, pos), reference_points, src, spatial_shapes, level_start_index, padding_mask)
